...
class CacheAndDatabaseTest extends TestCase
{
    public function test_metrics_are_cached_and_flushed_to_database()
    {
        ...
        $metrics = DB::connection('testing')->table($this->getMetricsTableName())->get();
    ...
    public function test_metrics_aggregation_in_cache()
    {
        ...
        $metric = DB::connection('testing')->table($this->getMetricsTableName())
    ...
    public function test_metrics_with_different_tags_are_separate()
    {
        ...
        $metrics = DB::connection('testing')->table($this->getMetricsTableName())
            ->where('name', 'test.counter')
            ->get();
    ...
    public function test_clean_old_metrics()
    {
        ...
        DB::connection('testing')->table($this->getMetricsTableName())->insert([
            [
                'name' => 'old.metric',
                'value' => 1.0,
                'tags' => json_encode([]),
        ...
        $remaining = DB::connection('testing')->table($this->getMetricsTableName())->count();
        $this->assertEquals(1, $remaining);

        $remainingMetric = DB::connection('testing')->table($this->getMetricsTableName())->first();
